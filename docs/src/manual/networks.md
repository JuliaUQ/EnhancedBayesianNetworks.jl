...
The reverse transition happens automatically: after reduction, a credal network whose imprecision has vanished is narrowed back to aBN.
...
An eBN is not queried directly, it is transformed into an inference-ready discrete network (BN or CN) by [`reduce`](@ref), which discretizes its continuous nodes and evaluates its functional nodes as structural reliability problems, yielding a BN or a CN. 
