# # Utility Functions
#
# A short tour of the structural and query helpers a network provides: the
# **Markov blanket** of a node, the **joint probability** of a full assignment, and
# drawing a **sample** from the network.

using EnhancedBayesianNetworks

# ## Markov blanket
#
# The [`markov_blanket`](@ref) of a node is the set that shields it from the rest of
# the network — its **parents**, its **children**, and its children's other parents
# (**co-parents**). Conditioned on its blanket, a node is independent of every other
# node. On the network below, the blanket of `x6` is its parents `x3`, `x4`, its
# children `x9`, `x10`, and their co-parents `x5`, `x8`.
...
# ## A fully specified sprinkler network
#
# For the remaining helpers we use a small, completely specified BN: the weather
# drives whether it rains and whether the sprinkler runs, and both wet the grass.
...
G[:R=>:yesR, :S=>:offS, :G=>:wetG] = 0.90; G[:R=>:yesR, :S=>:offS, :G=>:dryG] = 0.10
G[:R=>:noR,  :S=>:onS,  :G=>:wetG] = 0.90; G[:R=>:noR,  :S=>:onS,  :G=>:dryG] = 0.10
G[:R=>:noR,  :S=>:offS, :G=>:wetG] = 0.00; G[:R=>:noR,  :S=>:offS, :G=>:dryG] = 1.00
...
# ## Joint probability
#
# [`joint_probability`](@ref) evaluates the probability of a *complete* assignment of
# every node — one term of the network's factorized joint distribution. Here: a sunny
# day, no rain, sprinkler on, wet grass.

joint_probability(bn, Dict(:W => :sunny, :R => :noR, :S => :onS, :G => :wetG))

# ## Sample
#
# [`sample`](@ref) draws one realization from the network by ancestral sampling,
# returning a one-row table with a state for every node.
