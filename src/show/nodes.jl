...
function Base.show(io::IO, node::DiscreteNode)
    return print(io, "DiscreteNode(", node.name, ", parents=", parents(node), ", states=", states(node), ")")
end
function Base.show(io::IO, ::MIME"text/plain", node::DiscreteNode)
    ...
    println(io)
    return show(io, MIME"text/plain"(), node.cpt.data)
end
...
function Base.show(io::IO, node::ContinuousNode)
    return print(
        io, "ContinuousNode(", node.name, ", parents=", parents(node), ", discretization=", typeof(node.discretization).name.name, ")"
    )
end
...
function Base.show(io::IO, ::MIME"text/plain", node::ContinuousFunctionalNode)
    ...
    return if node.simulation isa ScenariosTable
        println(io)
...
function Base.show(io::IO, node::DiscreteFunctionalNode)
    return print(io, "DiscreteFunctionalNode(", node.name, ", states=", states(node), ", models=", length(node.models), ")")
end
