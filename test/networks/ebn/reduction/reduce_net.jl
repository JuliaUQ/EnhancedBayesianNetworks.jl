@testset "Evaluation Net" begin
    @testset "Structural System" begin
        ...
        @testset "No Discrete Parents - Precise" begin
            ...
            @test isapprox(ebn1.nodes[1].cpt.data.Π, [0.025693, 0.974307], atol=0.05)
            @test !isnothing(ebn1.nodes[1].results)

            ebn2 = deepcopy(ebn)
            @suppress ebn2 = reduce(ebn2, false)
            ...
            @test isapprox(ebn2.nodes[1].cpt.data.Π, [0.025693, 0.974307], atol=0.05)
