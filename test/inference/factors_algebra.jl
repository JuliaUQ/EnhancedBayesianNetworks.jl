...
@testitem "Factors Algebra - normalize" begin
    # normalize - 1D
    f = EnhancedBayesianNetworks.Factor([1], [2.0, 3.0])
    ...
    # normalize - 2D
    f = EnhancedBayesianNetworks.Factor([1, 2], [1.0 2.0; 3.0 4.0])
    n = EnhancedBayesianNetworks.normalize(f)
    @test sum(n.table) ≈ 1.0
    @test n.table ≈ [0.1 0.2; 0.3 0.4]

    # normalize - constant factor
    f = EnhancedBayesianNetworks.Factor(Int[], fill(5.0))
    n = EnhancedBayesianNetworks.normalize(f)
    @test n.vars == Int[]
    @test n.table[] ≈ 1.0
end